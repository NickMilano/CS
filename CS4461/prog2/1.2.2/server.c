      ...
      if ((numbytes = recvfrom(sockfd, buf, MAXBUFLEN-1 , 0,
          ...
          seqNum = getSeq(buf);
              ...
              if(strcmp(words[seqNum],buf) == 0){
                /*already had this packet ignore*/
                printf("server: duplicate packet. reACKing\n");
                sprintf(buff, "%d ACK", seqNum);
                if((numbytes = unreliable_sendto(sockfd, buff, strlen(buff), 0,
                        (struct sockaddr *) &their_addr, addr_len)) == -1) {
              ...
              if(seqNum == 1)
                ...
                expecting should be reset to 0 and we can just ACK this packet*/
              if(expecting == 0) {
                printf("server: did not receive SYN, ACKing dropped packet\n");
                sprintf(buff, "%d ACK", seqNum);
                if((numbytes = unreliable_sendto(sockfd, buff, strlen(buff), 0,
                        (struct sockaddr *) &their_addr, addr_len)) == -1) {
                        perror("talker: sendto");
