...
int main(int argc, char *argv[])
{
    ...
    /*Printing seqNum into array beforehand for simplicity*/
    for(i=0; i< 1000; i++) {
      sprintf(buf, "%d", i+1);
    ...
    while(j < packetsToSend || waitingAcks > 0){
      ...
      if(rv > 0) {
        ...
        if(acks[seqNum] == 1) {
          ...
          /*new ack*/
          acks[seqNum]=1; /*set this so we know this packet has been ACKd*/
          if(seqNum == expectedACK) {
            ...
            /*Got a packet out of order, expected must have dropped*/
            /*I made this logic initially for 1.2.1 as it seemed useful and it also solves 1.2.2*/
